...
parseCrate = many (try (string "[" *> satisfy isUpper <* string "] ") <|> try (string "    " *> pure '-'))

parseMove :: Parser (Int, Int, Int)
parseMove = (,,) <$> (string "move " *> integer) <*> (string " from " *> integer) <*> (string " to " *> integer)

parseAll :: Parser ([[Char]], [(Int, Int, Int)])
parseAll = (,) <$> (map (filter (/= '-')) . transpose <$> sepBy parseCrate newline <* manyTill (satisfy isDigit <|> char ' ') newline) <*> (newline *> sepBy parseMove newline)

move :: Bool -> [[Char]] -> (Int, Int, Int) -> [[Char]]
move part stacks (n, f, t) = let f' = drop n (stacks !! (f-1))
                                 t' = (if part then reverse . take n else take n) (stacks !! (f-1)) ++ (stacks !! (t-1)) in
